...
class Events extends Component {
  state = {registrationStatusValue: 'Initial'}

  checkStatus = id => {
    const filteredList = eventsList.filter(each => each.id === id)
    this.setState({registrationStatusValue: filteredList[0].registrationStatus})
  }

  render() {
    const {registrationStatusValue} = this.state
    return (
      <div className="bg-container">
        ...
        <div className="result-bg-container">
          <ActiveEventRegistrationDetails
            activeEventDetails={registrationStatusValue}
          />
        </div>
      </div>
    )
